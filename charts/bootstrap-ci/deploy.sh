...
echo ""
echo "You can add the following webhook on your github projects: "
echo "- http://$(kubectl -n tekton-pipelines get ingress el-ci-ingress -o="jsonpath={.spec.rules[0].host}")"
echo "- http://$(kubectl -n tekton-pipelines get ingress stargate -o="jsonpath={.spec.rules[0].host}")"/webhooks
echo ""
